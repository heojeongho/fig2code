...
- **토큰 절약**: 이미지(시안 풀샷·에셋)는 base64가 아닌 URL+curl로 디스크에 직행시키고, `get_design_context`는 `excludeScreenshot: true`, 한도 초과로 파일 저장된 대형 응답은 python으로 필요한 부분만 파싱한다. 히트맵은 상위 분면만 열람. (일반 MCP 원샷 대비 총 토큰 ≈1.3–1.6×, 추가분은 대부분 검증 루프 — README 참고)
